...
def MailNewIngresoMensualidad(usuario, monto, plan, correo):
	...
	mailsadmin = tb_mailsAdministrador.objects.all()
	if(len(mailsadmin) > 0 ):
		for i in range(0,len(mailsadmin)):
			...
			send_mail(email_subject_Soporte, cuerpo , 'musculando@b7000615.ferozo.com', ['soporte@apreciasoft.com', "mg.arreaza.13@gmail.com", mailsadmin[i].mail],fail_silently=True, html_message=email_body_Soporte)
...
def NewSocioMAil(usuario, monto, plan, correo, ):
	...
	if(len(mailsadmin) > 0 ):
		for i in range(0,len(mailsadmin)):
			...
			send_mail(email_subject_Soporte, cuerpo , 'musculando@b7000615.ferozo.com', ["mg.arreaza.13@gmail.com",mailsadmin[i].mail],fail_silently=True, html_message=email_body_Soporte)
...
def PagoReserva(usuario,correo):
	cuerpo = ""
	###Mensaje para el usuario #########
	email_subject_usuario = 'Musculando - Pago De Reserva'
	email_body_usuario = "Hola %s, Hemos resgistrado satisfactoriamente su pago de reserva!" %(usuario)
	send_mail (email_subject_usuario, cuerpo, 'musculando@b7000615.ferozo.com', [correo], fail_silently=True, html_message=email_body_usuario)
...
@app.task
def confirmaciondereserva(usuario,correo):
	cuerpo = ""
	###Mensaje para el usuario #########
	email_subject_usuario = 'Musculando - Reserva de Cancha'
	email_body_usuario = "Hola %s, Nos complace comunicarle que su reserva ha sido aprobada!" %(usuario)
	send_mail (email_subject_usuario, cuerpo, 'musculando@b7000615.ferozo.com', [correo], fail_silently=True, html_message=email_body_usuario)

@app.task
def negaciondereserva(usuario,correo):
	cuerpo = ""
	###Mensaje para el usuario #########
	email_subject_usuario = 'Musculando - Reserva de Cancha'
