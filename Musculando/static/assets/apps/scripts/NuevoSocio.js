...
function DescuentoSeleccionado(id) {
	var screen = $('#loading-screen');
    configureLoadingScreen(screen);
	$.ajax({
		    // la URL para la petición
		    url : '/Configuracion/Porcentaje/Solicitud/Get/Registro/',
		    // la información a enviar
		    // (también es posible utilizar una cadena de datos)
		    data : { 
		    	'id':id,
		    ...
		    success : function(descuento) {
		    	...
		    	if (descuento) {
		    		//todo correcto 
		    		$('#SeleccionarDescuento').addClass('hidden');
		    		$('#botonselectdescuento').addClass('hidden');
		    		$('#CartadeDescuento').removeClass('hidden');
		    		$('#TituloDescuent').html(descuento.descuento).append('%');
		    		
		    		$('#Descripciondescuento').html(descuento.descripcion);
		    		
		    		$('#IdDescuentoSeleccionado').val(descuento.descuento);
